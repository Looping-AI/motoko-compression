...
shared ({ caller = _owner }) persistent actor class Compression() = self {
  ...
  public func generateData(size_mb : Nat) : async () {
    let rng = Random.seed(42 : Nat64);
    _data := ?Array.tabulate<Nat8>(size_mb * MB, func(_) { rng.nat8() });
  };

  /// Fill the canister with exactly `n_bytes` of pseudo-random bytes.
  /// Intended for perf scripts that need sub-MiB workload sizes (e.g. when
  /// instrumenting low-level primitives where 1 MiB would flood the output).
  public func generateBytes(n_bytes : Nat) : async () {
    let rng = Random.seed(42 : Nat64);
    _data := ?Array.tabulate<Nat8>(n_bytes, func(_) { rng.nat8() });
  };
