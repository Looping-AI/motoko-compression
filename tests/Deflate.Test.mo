...
suite("Symbol length boundaries", func() {
...
suite("Symbol distance boundaries", func() {
...
suite("Fixed Huffman full-range round-trip", func() {

  test("all bytes 0..127 (8-bit literal range)", func() {
    let data = Array.tabulate<Nat8>(128, func(i) = Nat8.fromNat(i));
    expect.array(roundTrip(data, fixedOpts), Nat8.toText, Nat8.equal).equal(data);
  });

  test("all bytes 128..255 (9-bit literal range)", func() {
    let data = Array.tabulate<Nat8>(128, func(i) = Nat8.fromNat(128 + i));
    expect.array(roundTrip(data, fixedOpts), Nat8.toText, Nat8.equal).equal(data);
  });

  test("all bytes 0..255 (mixed 8-bit + 9-bit)", func() {
...
suite("Long-pointer round-trips", func() {

  // 300 identical bytes → LZSS will emit at least one pointer of length 258.
  test("300×A (exercises max length 258)", func() {
  ...
  test("1024 byte periodic pattern (16-byte period)", func() {
  ...
  // far-back position.
  test("2048 bytes ascending mod 251 (no long matches)", func() {
...
suite("Multi-block round-trips", func() {

  test("raw, 200 bytes, block_size=37 (forces ~5 blocks)", func() {
    let data = Array.tabulate<Nat8>(200, func(i) = Nat8.fromNat(i % 256));
    let opts : Deflate.DeflateOptions = { block_size = 37; dynamic_huffman = false; lzss = null };
    expect.array(roundTrip(data, opts), Nat8.toText, Nat8.equal).equal(data);
  });

  test("fixed Huffman, 1000 bytes, block_size=100", func() {
    let data = Array.tabulate<Nat8>(1000, func(i) = Nat8.fromNat(i % 256));
    let opts : Deflate.DeflateOptions = { block_size = 100; dynamic_huffman = false; lzss = ?#fast };
    expect.array(roundTrip(data, opts), Nat8.toText, Nat8.equal).equal(data);
  });

  test("dynamic Huffman, 500 bytes, block_size=64", func() {
    let data = Array.tabulate<Nat8>(500, func(i) = Nat8.fromNat((i * 7) % 256));
    let opts : Deflate.DeflateOptions = { block_size = 64; dynamic_huffman = true; lzss = ?#best };
    expect.array(roundTrip(data, opts), Nat8.toText, Nat8.equal).equal(data);
  });
});
...
suite("Decoder error paths", func() {

  test("invalid BTYPE = 3 rejected", func() {
    // First byte = 0b00000111: BFINAL=1, BTYPE=11 (bits laid LSB-first).
    expectErr(decodeBytes([0x07]), "BTYPE=3");
  });

  test("raw block with LEN/NLEN mismatch", func() {
  ...
  test("empty bitstream is treated as truncated", func() {
    expectErr(decodeBytes([]), "empty stream");
  });

  test("non-final raw block followed by EOF is truncated", func() {
  ...
  test("two empty raw blocks (non-final + final) decode to []", func() {
    let bytes : [Nat8] = [
      0x00, 0x00, 0x00, 0xFF, 0xFF,   // non-final empty raw
      0x01, 0x00, 0x00, 0xFF, 0xFF,   // final empty raw
    ];
...
suite("Encoder construction", func() {

  test("raw block_size = 65535 accepted", func() {
    let opts : Deflate.DeflateOptions = { block_size = 65_535; dynamic_huffman = false; lzss = null };
    let _ = Deflate.buildEncoder(opts);
  });

  test("compressed block_size > 65535 accepted", func() {
    let opts : Deflate.DeflateOptions = { block_size = 100_000; dynamic_huffman = true; lzss = ?#fast };
    let _ = Deflate.buildEncoder(opts);
  });
});
...
suite("Dynamic Huffman edge cases", func() {

  test("input with zero pointers (only literals, distance tree degenerate)", func() {
  ...
  test("highly repetitive input (long zero-runs in bitwidth tables)", func() {
