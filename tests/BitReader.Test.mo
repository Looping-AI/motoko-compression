...
suite("peekBit / readBit", func() {
  test("peekBit returns correct first bit", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    expect.bool(r.peekBit()).isTrue();
  });

  test("peekBit is idempotent (does not advance)", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    ignore r.peekBit();
    ignore r.peekBit();
  ...
  test("readBit advances offset by 1", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    ignore r.readBit();
    expect.nat(r.getPosition()).equal(1);
  });

  test("readBit sequence reads LSB-first bits of 0xA5", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
...
suite("peekBits / readBits", func() {
  test("peekBits(3) on 0xA5 = 5 (bits: 1,0,1 → 1+0+4)", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    expect.nat(r.peekBits(3)).equal(5);
  });

  test("peekBits is non-mutating", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    let v1 = r.peekBits(3);
    let v2 = r.peekBits(3);
...
suite("skipBits", func() {
  test("skipBits advances offset", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    r.skipBits(3);
    expect.nat(r.getPosition()).equal(3);
  });

  test("skipBits then readBit reads correct bit", func() {
...
suite("peekByte / readByte", func() {
  test("readByte on 0xA5 = 165", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    expect.nat8(r.readByte()).equal(0xA5);
  });

  test("peekByte does not advance offset", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    ignore r.peekByte();
    expect.nat(r.getPosition()).equal(0);
  });

  test("peekByte and readByte return the same value", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
  ...
  test("sequential readByte reads two bytes", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8, 0x42 : Nat8]);
    expect.nat8(r.readByte()).equal(0xA5);
...
suite("peekBytes / readBytes", func() {
  test("peekBytes is non-mutating", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8, 0x42 : Nat8]);
  ...
  test("readBytes reads correct values and advances", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8, 0x42 : Nat8]);
  ...
  test("readBytes clamps to available byteSize", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
...
suite("byteAlign", func() {
  test("byteAlign after 1 bit consumed skips 7 bits to empty", func() {
  ...
  test("byteAlign on byte-aligned data is a no-op", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    r.byteAlign(); // bitSize=8, 8%8=0 → no change
    expect.nat(r.bitSize()).equal(8);
  });

  test("byteAlign leaves clean multiple of 8 bits", func() {
...
suite("hideTailBits / showTailBits / hiddenTailBits", func() {
  test("hideTailBits reduces visible bitSize", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    r.hideTailBits(2);
  ...
  test("showTailBits restores full bitSize", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    r.hideTailBits(2);
    r.showTailBits();
    expect.nat(r.bitSize()).equal(8);
...
suite("getPosition / setPosition", func() {
  test("seek back and re-read same byte", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8, 0x42 : Nat8]);
...
suite("reset", func() {
  test("reset restores offset to 0, data preserved", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    ignore r.readBits(5);
    r.reset();
    expect.nat(r.getPosition()).equal(0);
...
suite("clearRead", func() {
  test("clearRead drops consumed bits, offset resets to 0", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8, 0x42 : Nat8]);
...
suite("clear", func() {
  test("clear empties buffer completely", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8, 0x42 : Nat8]);
    r.clear();
    expect.nat(r.bitSize()).equal(0);
    expect.nat(r.getPosition()).equal(0);
...
suite("addBytes / fromBytes", func() {
  test("addBytes appends data for reading", func() {
  ...
  test("fromBytes initialises reader with data", func() {
    let r = BitReader.fromBytes([0xA5 : Nat8]);
    expect.nat(r.bitSize()).equal(8);
    expect.nat8(r.readByte()).equal(0xA5);
  });
