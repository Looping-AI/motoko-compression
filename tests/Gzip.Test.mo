...
suite("Fixed Huffman round-trips", func() {
  ...
  test("1 KB repeated bytes (high compression ratio)", func() {
    let data = Array.tabulate<Nat8>(1024, func(_) { 0xAA });
    expect.array(roundTrip(data, defaultBuilder()), Nat8.toText, Nat8.equal).equal(data);
  });

  test("all 256 byte values", func() {
...
suite("Header fields", func() {

  test("OS byte preserved", func() {
...
suite("Multi-chunk output", func() {

  test("small block size produces multiple chunks", func() {
  ...
  test("total_size matches sum of chunk sizes", func() {
...
suite("Error handling", func() {

  test("invalid magic bytes → #err", func() {
    let decoder = Gzip.Decoder();
  ...
  test("CRC32 mismatch → #err", func() {
    ...
    let corrupted = Array.tabulate<Nat8>(all.size(), func(i) {
