...
suite(
  ...
  func() {
    ...
    test(
      "empty input returns 0x00000000 (standard CRC-32)",
      func() {
    ...
    test(
      "single byte 0x61 ('a') returns 0xE8B7BE43",
      func() {
    ...
    test(
      "\"123456789\" returns 0xCBF43926 (canonical IEEE test vector)",
      func() {
    ...
    test(
      "17-byte input exercises singleSlicingUpdate path",
      func() {
...
suite(
  ...
  func() {
    ...
    test(
      "update then finish matches checksum",
      func() {
        let data : [Nat8] = [0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39];
        let crc = CRC32.CRC32();
        crc.update(data);
    ...
    test(
      "updateByte one-by-one matches update all-at-once",
      func() {
    ...
    test(
      "reset then finish returns empty CRC 0x00000000",
      func() {
        let crc = CRC32.CRC32();
    ...
    test(
      "finish resets state automatically",
      func() {
        let data : [Nat8] = [0x61];
        let crc = CRC32.CRC32();
        crc.update(data);
    ...
    test(
      "reusable after finish: second checksum is correct",
      func() {
        let crc = CRC32.CRC32();
        crc.update([0x61]);
        let _ = crc.finish();
