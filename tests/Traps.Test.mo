...
persistent actor {
  ...
  public func runTests() : async () {
    ...
    await suite(
      "BitReader traps — bounds checking",
      func() : async () {

        await test(
          "peekBit traps on empty reader",
          func() : async () {
            var trapped = false;
        ...
        await test(
          "readBits(9) traps when only 8 bits available",
          func() : async () {
            var trapped = false;
    ...
    await suite(
      "LZSS decoder traps — invalid pointers",
      func() : async () {

        await test(
          "decode #pointer with backward_offset > output size traps",
          func() : async () {
            var trapped = false;
